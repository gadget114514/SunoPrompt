...
- **6つのカテゴリから選択** — ジャンル・ボーカル・楽器・コード・ムード・構造を組み合わせてプロンプトを生成
...
- **タブの並べ替え** — カテゴリタブをドラッグで移動でき、タブの並び順がそのままプロンプトの記述順になります。BPM チップも一緒に並べ替えられ、テンポの位置を自由に決められます
...
カテゴリタブを左右にドラッグする（またはタブにフォーカスして Ctrl + ← / →）と、その
カテゴリがプロンプトのどこに入るかを変更できます。同じ行にある **BPM** チップも同様に動かせるので、
テンポを先頭・末尾・途中のどこにでも置けます。BPM の値は従来どおり上部バーのスライダーで設定し、
チップをクリックしても表示中のタブは切り替わりません。並び順は次回起動時にも保持され、
プロジェクトにも保存されます。

🎲 ランダム生成の横にある **▾** ボタンでカテゴリ別の設定を開閉できます（既定は閉じた状態）。
...
| `suno_mood_phrases.json` | ムード・感情・音の質感 |
